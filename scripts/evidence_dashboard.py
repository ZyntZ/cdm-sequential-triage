...
from confirmation import (
    file_sha256, read_confirmation_status, read_json,
    validate_calibration_artifact,
)
from validation_plan import maximum_passing_failures, pass_probability, cp_upper
...
def validate_planning_table(
    ...
    *,
    ...
    """Recompute every finite-sample claim displayed from the planning CSV."""
    probability_column = f"pass_probability_if_true_rate_{assumed_rate:.2f}"
    required = {
        "positive_events", "maximum_passing_failures",
        "upper_bound_at_maximum", probability_column,
    }
    missing = required.difference(plan.columns)
    if missing:
        raise ValueError(f"Missing planning columns: {sorted(missing)}")
    if plan.empty or plan["positive_events"].duplicated().any():
        raise ValueError("Planning table must contain unique positive-event counts")
    for _, row in plan.iterrows():
        n_positive = int(row["positive_events"])
        expected_failures = maximum_passing_failures(
            n_positive, alpha=alpha, confidence=confidence
        )
        observed_failures = int(row["maximum_passing_failures"])
        if observed_failures != expected_failures:
            raise ValueError("Planning maximum-passing-failures mismatch")
        expected_bound = (
            None if expected_failures < 0
            else cp_upper(expected_failures, n_positive, confidence)
        )
        observed_bound = row["upper_bound_at_maximum"]
        if expected_bound is None or not abs(float(observed_bound) - expected_bound) <= 1e-12:
            raise ValueError("Planning upper-bound mismatch")
        expected_probability = pass_probability(
            n_positive, assumed_rate, alpha=alpha, confidence=confidence
        )
        observed_probability = row[probability_column]
        if not abs(float(observed_probability) - expected_probability) <= 1e-12:
            raise ValueError("Planning pass-probability mismatch")
...
def localize_evidence_document(document: str, locale: str) -> str:
    ...
    replacements = [
        ...
        ("Scientific evidence · not for operations", "Научные результаты · не для эксплуатации"),
        ("Sequential CDM triage · evidence", "Последовательный триаж CDM · результаты"),
        ("Development evidence", "Результаты разработки"),
        ...
        ("Fold-level safety diagnostic", "Диагностика безопасности по folds"),
        ("folds meet the pooled 10% UCB criterion", "folds выполняют объединённый критерий UCB 10%"),
        ("Each fold contains only", "Каждый fold содержит только"),
        ("positive events, so these intervals are imprecise and are not subgroup guarantees.", "положительных событий, поэтому интервалы неточны и не являются гарантиями для подгрупп."),
        ...
        ("Confirmation v1 · CRITERION NOT MET", "Confirmation v1 · критерий не выполнен"),
        ("Confirmation v1 · CRITERION MET", "Confirmation v1 · критерий выполнен"),
        ...
        ("Correct exclusions per 1,000", "Корректных исключений на 1 000"),
        ("Remaining per 1,000", "Остаётся на 1 000"),
        ("Historical queue effect", "Исторический эффект для очереди"),
        ("correct SAFE-EXCLUDE decisions per 1,000 evaluated events", "корректных решений SAFE-EXCLUDE на 1 000 оценённых событий"),
        ("This is a retrospective count, not a staffing or monetary claim.", "Это ретроспективный подсчёт, а не оценка штата или денежного эффекта."),
        ...
        ("Preregistered next study · outcomes not accessed", "Следующее исследование · исходы не открыты"),
...
def build_evidence_dashboard(root:Path,output:Path,locale:str="en")->dict[str,Any]:
    ...
    confirmation_status=read_confirmation_status(
        c/'confirmation.lock', c/'confirmation.json'
    )
    if confirmation_status['status'] not in {'completed','legacy-completed'}:
        raise ValueError(
            'confirmation_v1 is not in a terminal completed state '
            f"(status={confirmation_status['status']!r}); dashboard cannot be rendered"
        )
    ...
    oof_verification=verify_oof_evidence(
        a/'development_event_aligned_oof_v8.parquet',
        a/'development_event_aligned_v8.json',
        a/'catboost_tail_aligned_final_v13.json',
        a/'catboost_tail_aligned_final_v13.cbm',
        a/'next_validation_preregistration_v12.json',
        a/'next_validation_preregistration_v12.lock',
    )
    if not oof_verification['passed']:
        failed=', '.join(
            check['name'] for check in oof_verification['checks']
            if not check['passed']
        )
        raise ValueError(f"OOF evidence verification failed: {failed}")
    ...
    criterion=float(conf['policy']['alpha'])
    confidence=float(conf['policy']['confidence'])
    validate_planning_table(plan, alpha=criterion, confidence=confidence)
    passed=float(conf['evaluation']['danger_ucb'])<=criterion
    ...
    tail_folds=folds.loc[folds['method'].eq('catboost_tail_aligned')].copy()
    live_folds={row['fold']: row for row in oof_verification['fold_results']}
    fold_fields=('danger_k','danger_n','danger_rate','danger_ucb','safe_negative','negative_n','safe_negative_rate','median_first_safe_tca_days')
    for row in tail_folds.itertuples(index=False):
        live=live_folds.get(int(row.fold))
        if live is None:
            raise ValueError(f"fold {int(row.fold)} missing from live OOF verification")
        for field in fold_fields:
            stored=getattr(row, field)
            observed=live[field]
            if isinstance(observed, int):
                matches=int(stored)==observed
            else:
                matches=abs(float(stored)-float(observed))<=1e-12
            if not matches:
                raise ValueError(
                    f"fold {int(row.fold)} {field} differs from live OOF verification"
                )
    folds_hash=file_sha256(r/'development_score_ensemble_folds_v10.csv')
    oof_checks=len(oof_verification['checks'])
    oof_passed=sum(check['passed'] for check in oof_verification['checks'])
    fold_safety_passes=int(tail_folds['danger_ucb'].le(criterion).sum())
    fold_safety_total=int(len(tail_folds))
    fold_positive_min=int(tail_folds['danger_n'].min())
    fold_positive_max=int(tail_folds['danger_n'].max())
    evaluation_events=int(conf['evaluation_events'])
    safe_per_1000=1000.0*int(m['safe_negative'])/evaluation_events
    remaining_per_1000=1000.0*(evaluation_events-int(m['safe_negative']))/evaluation_events
    css="body{margin:0;background:#f4f5f7;color:#20242a;font:14px/1.5 -apple-system,BlinkMacSystemFont,Segoe UI,Arial,sans-serif}main{max-width:1200px;margin:auto;padding:24px}h1{margin:6px 0 18px;font-size:25px;font-weight:600}h2{font-size:19px;margin:4px 0 14px}h3{font-size:16px;margin:18px 0 8px}.tier{min-width:0;border:1px solid #d8dce2;border-left:4px solid;padding:18px;margin:14px 0;background:#fff}.dev{border-left-color:#68707c}.conf{border-left-color:#a12638}.next{border-left-color:#315f8c}.tag{font-size:12px;font-weight:600;color:#68707c}.dev .tag{color:#59616d}.conf .tag{color:#8f1e2f}.next .tag{color:#315f8c}.fail{background:#fff7f8;border:1px solid #d8a4ac;padding:11px;color:#7f2432;font-weight:600}table{width:100%;border-collapse:collapse;margin:10px 0}th,td{padding:8px;border-bottom:1px solid #e4e7eb;text-align:left;vertical-align:top;overflow-wrap:anywhere}th{color:#68707c;font-size:11px;font-weight:600}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:9px}.kpi{min-width:0;background:#f7f8fa;padding:11px;border:1px solid #e4e7eb}.kpi strong,.kpi span{display:block}.kpi strong{font-size:20px;overflow-wrap:anywhere}.caveat{color:#4b5563;border-left:3px solid #a12638;padding:9px 11px;background:#fff7f8;overflow-wrap:anywhere}.hash{font:11px ui-monospace,SFMono-Regular,Consolas,monospace;color:#68707c;overflow-wrap:anywhere}.evidence-plot{width:100%;height:auto;background:#fff;border:1px solid #d8dce2;margin:10px 0}.plot-bg{fill:#fff}.gridline{stroke:#e4e7eb;stroke-width:1}.axis-label,.point-label,.criterion-label{fill:#59616d;font:12px Arial,sans-serif}.axis-title{fill:#20242a;font:13px Arial,sans-serif}.criterion{stroke:#a12638;stroke-width:2;stroke-dasharray:7 5}.criterion-label{fill:#8f1e2f}.point{fill:#9aa1aa;stroke:#fff;stroke-width:1}.point.pareto{fill:#315f8c;stroke:#234664;stroke-width:2}.point.infeasible{fill:#a12638}.zero-line{stroke:#68707c;stroke-width:2}.ci-line{stroke:#68707c;stroke-width:2}.fold-point{stroke:#fff;stroke-width:1}.fold-point.gain{fill:#246b45}.fold-point.loss{fill:#a12638}.finding{background:#f7f8fa;border:1px solid #d8dce2;padding:11px;margin:10px 0;overflow-wrap:anywhere}@media(max-width:700px){main{padding:12px}.grid{grid-template-columns:1fr}.tier{padding:14px}table{display:block;overflow-x:auto}}@media print{body{background:#fff;color:#000}main{max-width:none;padding:0}.tier{background:#fff;border:1px solid #aaa;break-inside:avoid}.kpi,.finding{background:#fff}.fail,.caveat{background:#fff;border-color:#8f1e2f}.hash{font-size:9px}.evidence-plot{border-color:#aaa}.plot-bg{fill:#fff}.gridline{stroke:#ccc}.axis-label,.point-label,.axis-title{fill:#222}.criterion{stroke:#000}.criterion-label{fill:#000}}"
    doc=f"""<!doctype html><html lang='en'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width'><title>CDM Triage Evidence</title><style>{css}</style></head><body><main><div class='tag'>Scientific evidence · not for operations</div><h1>Sequential CDM triage · evidence</h1>
<section id='development' class='tier dev'><div class='tag'>Development evidence</div><h2>Safety–automation frontier</h2>{frontier}<div class='finding'><strong>Selected development candidate:</strong> tail-aligned CatBoost retained 12/192 dangerous exclusions (UCB95 {pct(tail['danger_ucb'])}) and increased correct SAFE-EXCLUDE coverage by {100*tail['coverage_delta']:.2f} percentage points (95% CI {100*tail['coverage_delta_ci_low']:.2f} to {100*tail['coverage_delta_ci_high']:.2f}; exact McNemar p={tail['coverage_mcnemar_p']:.3g}).</div><table><tr><th>Method</th><th>Danger</th><th>UCB95</th><th>Safe-negative</th><th>Feasible</th><th>Pareto</th></tr>{devrows}</table><h3>Outer-fold stability of the selected candidate</h3>{foldplot}<div class='finding'><strong>Fold-level safety diagnostic:</strong> {fold_safety_passes}/{fold_safety_total} folds meet the pooled 10% UCB criterion. Each fold contains only {fold_positive_min}–{fold_positive_max} positive events, so these intervals are imprecise and are not subgroup guarantees.</div><p class='caveat'>The pooled automation gain is heterogeneous: folds 1–3 improve, while folds 0 and 4 decline. This is reported as a limitation and motivates the genuinely new validation study; no subgroup-specific safety guarantee is claimed.</p><h3>Live OOF evidence verification</h3><div class='finding'><strong>{oof_passed}/{oof_checks} integrity and metric checks passed</strong> · verifier schema {oof_verification['schema_version']}. Stored fold diagnostics were cross-checked against decisions recomputed directly from the committed OOF scores.</div><p class='caveat'>{esc(oof_verification['note'])}</p><h3>250 correlated calibration-split repeats</h3><table><tr><th>Method</th><th>Repeats</th><th>Median coverage</th><th>Coverage gain &gt; 0</th><th>Danger not worse</th></tr>{stab}</table><p class='caveat'>Development data only: 7,146 events, 192 positives. Fixed out-of-fold scores were reused across correlated calibration splits; these are not independent retraining replications and are not confirmation evidence.</p><div class='hash'>v10 {file_sha256(a/'development_score_ensemble_v10.json')} · v11 {file_sha256(a/'development_score_ensemble_repeated_v11.json')}</div></section>
<section id='confirmation' class='tier conf'><div class='tag'>Confirmation v1 · {'CRITERION MET' if passed else 'CRITERION NOT MET'}</div><h2>Snapshot-model confirmation</h2><div class='fail'>PRE-SPECIFIED CRITERION {'MET' if passed else 'NOT MET'}: UCB {pct(m['danger_ucb'])} {'≤' if passed else '>'} α {pct(criterion)}.</div><div class='grid'><div class='kpi'><span>Dangerous exclusions</span><strong>{m['danger_k']}/{m['danger_n']}</strong></div><div class='kpi'><span>Observed danger</span><strong>{pct(m['danger_rate'])}</strong></div><div class='kpi'><span>Safe-negative</span><strong>{pct(m['safe_negative_rate'])}</strong></div><div class='kpi'><span>Median lead</span><strong>{m['median_first_safe_tca']:.2f} d</strong></div><div class='kpi'><span>Correct exclusions per 1,000</span><strong>{safe_per_1000:.0f}</strong></div><div class='kpi'><span>Remaining per 1,000</span><strong>{remaining_per_1000:.0f}</strong></div></div><div class='finding'><strong>Historical queue effect:</strong> {safe_per_1000:.0f} correct SAFE-EXCLUDE decisions per 1,000 evaluated events, at a median lead of {m['median_first_safe_tca']:.2f} days. This is a retrospective count, not a staffing or monetary claim.</div><p>PAC rank {cr['rank']}/{cr['n_positive']} · threshold {cr['threshold']:.8g} · calibration bound {pct(cr['pac_bound'])}</p><p class='caveat'>Single immutable historical run for catboost_snapshot. No second run is permitted. This result does not validate the preregistered v13 candidate.</p><div class='hash'>confirmation {file_sha256(c/'confirmation.json')} · lock {file_sha256(c/'confirmation.lock')} · model {cal['model_sha256']}</div></section>
<section id='preregistered' class='tier next'><div class='tag'>Preregistered next study · outcomes not accessed</div><h2>{esc(cand['score'])}</h2><div class='grid'><div class='kpi'><span>Status</span><strong>{esc(pre['status'])}</strong></div><div class='kpi'><span>Calibration positives</span><strong>≥{ns['recommended_calibration_positive_events']}</strong></div><div class='kpi'><span>Evaluation positives</span><strong>≥{ns['recommended_evaluation_positive_events']}</strong></div><div class='kpi'><span>Threshold</span><strong>NOT SET</strong></div></div><p>{esc(cand['model'])}; hard fraction {cand['hard_fraction']}, hard mass {cand['hard_mass']}, {cand['iterations']} iterations, window {cand['decision_window_days'][0]}–{cand['decision_window_days'][1]} d, minimum history {cand['minimum_history']}.</p><h3>Prospective evaluation planning</h3><table><tr><th>Positive events</th><th>Max failures</th><th>UCB at max</th><th>P(pass | true danger=5%)</th></tr>{prow}</table><p class='caveat'>confirmation_v1 was previously unblinded and cannot be reused. v13 has calibration_accessed=false, evaluation_accessed=false, and threshold=null. A genuinely new, disjoint study must be frozen before outcomes are opened.</p><div class='hash'>preregistration {prehash} · planning {planhash} · v13 model {modelhash}</div></section>
<footer class='caveat'>Dataset: ESA Collision Avoidance Challenge, Zenodo 10.5281/zenodo.4463683, CC BY 4.0. Target is high final calculated collision probability, not collision occurrence. Event-level exchangeability is required; no operational guarantee is claimed under arbitrary shift.</footer></main></body></html>"""
    ...
    return {'confirmation_passed':passed,'danger_ucb':float(m['danger_ucb']),'criterion':criterion,'historical_correct_safe_excludes_per_1000':safe_per_1000,'historical_remaining_per_1000':remaining_per_1000,'candidate_fold_safety_passes':fold_safety_passes,'candidate_fold_safety_total':fold_safety_total,'development_pareto_methods':[x['method'] for x in v10['summary'] if x['pareto_frontier']],'preregistration_frozen':pre['status']=='frozen-before-new-data','next_study_candidate':cand['score'],'calibration_accessed':v13['calibration_accessed'],'evaluation_accessed':v13['evaluation_accessed'],'v13_threshold':v13['threshold'],'confirmation_terminal_status':confirmation_status['status'],'oof_verification_passed':oof_verification['passed'],'oof_verification_checks':oof_checks,'oof_verification_schema':oof_verification['schema_version'],'oof_fold_crosscheck_passed':True,'folds_csv_sha256':folds_hash,'oof_fold_results':oof_verification['fold_results'],'locale':locale}

...
- Verify the committed v13 development evidence without retraining: `python scripts/verify_oof_evidence.py --output oof-verification.json`. The schema-v2 result includes pooled metrics and a machine-readable table for all five held-out folds. The command checks frozen artifact bindings, OOF structure, candidate parameters, and recomputes all published event-level metrics fold by fold. The evidence dashboard runs the same verifier before rendering and displays its fold results. This reads development artifacts only; it is not confirmation evidence, and the OOF score file itself was not cryptographically frozen before the verifier was added.
...
- One-shot confirmation: `python scripts/confirm_policy.py confirm --scores artifacts/new_evaluation_scores.parquet --labels data/new/evaluation_labels.parquet --model-manifest artifacts/catboost_tail_aligned_final_v13.json --study-manifest artifacts/new_study.json --study-lock artifacts/new_study.lock --calibration calibration.json --output confirmation.json --lock confirmation.lock [--gate artifacts/shift_gate.json]`. Before creating the irreversible confirmation lock, the command performs a label-blind preflight over the evaluation scores, frozen-study binding, calibration/model/gate identities, required columns, model hash, duplicate updates, calibration/evaluation event overlap, and internal consistency of the calibration rank and statistical bounds. Invalid label-blind inputs therefore do not consume the one permitted confirmation run. The result carries the calibration rank, sample size, marginal bound, PAC bound, and bound-status flag alongside the evaluation metrics. Label-roster validation remains after lock acquisition because it accesses evaluation outcomes.

...
- Verify the committed v13 development evidence without retraining: `python scripts/verify_oof_evidence.py --output oof-verification.json`. The command checks frozen artifact bindings, OOF structure, candidate parameters, and recomputes all published event-level metrics fold by fold. It reads development artifacts only. The result is not confirmation evidence, and the OOF score file itself was not cryptographically frozen before this verifier was added.
